...
class MidtransWebhookController extends Controller
{
    public function handle(Request $request)
    {
        ...
        // 🛠️ TAMBAHKAN BARIS INI UNTUK BYPASS WARNING PAGE VS CODE DEVTUNNELS
        if ($request->hasHeader('X-Tunnel-Skip-AntiPhishing-Page') || $request->hasHeader('X-Ngrok-Skip-Browser-Warning')) {
            header('X-Tunnel-Skip-AntiPhishing-Page: true');
            header('X-Ngrok-Skip-Browser-Warning: true');
        }

        $payload = $request->all();
        // ... (Sisa kode controller Anda ke bawah tetap sama seperti sebelumnya)
        $payload = $request->all();
