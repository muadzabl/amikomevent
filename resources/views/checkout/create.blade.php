    ...
    <div class="grid grid-cols-1 gap-8">
        ...
        <div class="bg-white rounded-3xl border border-slate-200 p-8 shadow-sm">
            ...
            <div class="flex gap-6 items-start">
                @php
                    $posterUrl = \Illuminate\Support\Str::startsWith($event->poster_path, ['http://', 'https://'])
                        ? $event->poster_path
                        : ($event->poster_path && \Illuminate\Support\Facades\Storage::disk('public')->exists($event->poster_path)
                            ? asset('storage/' . $event->poster_path)
                            : 'https://placehold.co/200x200/eef2ff/6366f1?text=No+Image');
                @endphp
                <img src="{{ $posterUrl }}" alt="Event" class="w-24 h-24 rounded-2xl object-cover">
                <div>
